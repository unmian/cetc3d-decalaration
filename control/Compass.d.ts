 ...
 * @LastEditors: Quarter
 * @LastEditTime: 2022-12-04 10:37:59
...
export interface Compass {
  // 当前类支持的EventType事件类型
  readonly EventType: CompassEventTypeCollection;
  // 当前控件的DOM对象
   ...
   * @param {Compass} propagate 将事件传播给父类 (用addEventParent设置)
   * @return {this}
   */
  fire(type: EventType, data: any, propagate?: Compass): this;
   ...
   * @param {EventType} type 事件类型
   * @param {Compass} propagate 是否判断指定的父类 (用addEventParent设置的)
   * @return {boolean}
   */
  listens(type: EventType, propagate?: Compass): boolean;
...
// 构造配置项
export interface ConstructorOptions extends BaseControlConstructorOptions {
  // 可选css定位top位置
  top?: string;
  // css定位bottom位置
  bottom?: string;
...
// // 支持事件类型集合
type CompassEventTypeCollection = BaseControl["EventType"];

// 支持事件类型
type EventType = CompassEventTypeCollection[keyof CompassEventTypeCollection];
